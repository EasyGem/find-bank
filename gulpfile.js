...
var path = {
  ...
  src: {
    articles: 'src/article/*.pug',
    html: 'src/*.pug',
    js: 'src/js/*.js',
    style: 'src/style/*.sass',
    libs: 'src/style/libs/*.css',
    img: 'src/img/**/*.*',
    fonts: 'src/fonts/**/*.*',
...
gulp.task('articles:build', function() {
  gulp
    .src(path.src.articles) //Выберем файлы по нужному пути
    .pipe(plumber())
    .pipe(rigger()) //Прогоним через rigger
    .pipe(pug()) // remove if use HTML
    .pipe(gulp.dest(path.build.articles)) //Выплюнем их в папку build
    .pipe(
      reload({
        stream: true
      })
...
gulp.task('libs:build', function() {
  gulp
    .src(path.src.libs) //Выберем файлы по нужному пути
    .pipe(gulp.dest(path.build.libs)) //Выплюнем их в папку build
    .pipe(cssmin())
    .pipe(
      reload({
        stream: true
